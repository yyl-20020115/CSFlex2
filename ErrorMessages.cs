...
public class ErrorMessages
{
    ...
    private static readonly Hashtable resources = [];

    static ErrorMessages()
    {
        var assembly = typeof(ErrorMessages).Assembly;
        var resx_stream = assembly.GetManifestResourceStream("csflex.Messages.xml");

        var reader = new ResXResourceReader(resx_stream);

        var e = reader.GetEnumerator();

        while (e.MoveNext())
            resources[e.Key] = e.Value;
